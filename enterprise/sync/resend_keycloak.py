...
# However, HTTP client errors (connection failures, timeouts) are wrapped by the Resend SDK
# with code=500 and error_type="HttpClientError" - these are handled separately below.
...
def _is_retryable_resend_error(exception: BaseException) -> bool:
    ...
    - Rate limit errors (429) via RateLimitError
    - Gateway errors (502, 503, 504) via ResendError with status code
    - HTTP client errors (connection failures, timeouts) via ResendError with
      error_type="HttpClientError" - the Resend SDK wraps all HTTP client
      exceptions (including requests.RequestException) in ResendError

    Note: The Resend SDK uses the 'requests' library internally, and wraps all
    HTTP client errors in ResendError with code=500 and error_type="HttpClientError".
    ...
    # Check for ResendError with retryable status codes or HTTP client errors
    if isinstance(exception, ResendError):
        # Check for HTTP client errors (connection failures, timeouts)
        ...
        if (
            ...
            and exception.error_type == "HttpClientError"
        ):
            logger.warning(f"Retrying due to HTTP client error: {exception}")
            return True
