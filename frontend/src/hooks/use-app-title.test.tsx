...
describe("useAppTitle", () => {
  ...
  it("should return 'Codeio' if is OSS and NOT in /conversations", async () => {
    // @ts-expect-error - only returning partial config for test
    ...
    await waitFor(() => expect(result.current).toBe("Codeio"));
  });

  it("should return 'Codeio Cloud' if is SaaS and NOT in /conversations", async () => {
    // @ts-expect-error - only returning partial config for test
    getConfigSpy.mockResolvedValue({
      app_mode: "saas",
    });
    ...
    await waitFor(() => expect(result.current).toBe("Codeio Cloud"));
  });

  it("should return '{some title} | Codeio' if is OSS and in /conversations", async () => {
    // @ts-expect-error - only returning partial config for test
    ...
    await waitFor(() =>
      expect(result.current).toBe("My Conversation | Codeio"),
    );
  });

  it("should return '{some title} | Codeio Cloud' if is SaaS and in /conversations", async () => {
    // @ts-expect-error - only returning partial config for test
    ...
    await waitFor(() =>
      expect(result.current).toBe(
        "Another Conversation Title | Codeio Cloud",
      ),
    );
  });
