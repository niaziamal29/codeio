...
const isCodeioEvent = (event: unknown): event is CodeioParsedEvent =>
  typeof event === "object" &&
  event !== null &&
  "id" in event &&
  "source" in event &&
...
const isCommandAction = (event: CodeioParsedEvent): event is CommandAction =>
...
export function WsClientProvider({
  ...
  function handleMessage(event: Record<string, unknown>) {
    ...
    if (isCodeioEvent(event)) {
      ...
      if (isCodeioAction(event) || isCodeioObservation(event)) {
        addEvent(event); // Event is already CodeioParsedEvent
      }
