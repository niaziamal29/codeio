 ...
 */
function groupEventsByPhase(events: CodeioEvent[]): CodeioEvent[][] {
  const phases: CodeioEvent[][] = [];
  let currentPhase: CodeioEvent[] = [];
 ...
 */
export function usePlanPreviewEvents(allEvents: CodeioEvent[]): Set<string> {
  return useMemo(() => {
