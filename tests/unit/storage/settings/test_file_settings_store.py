...
async def test_load_nonexistent_data(file_settings_store):
    with patch(
        ...
        MagicMock(return_value=CodeioConfig()),
    ):
...
async def test_get_instance():
    config = CodeioConfig(file_store='local', file_store_path='/test/path')

    with patch(
        'codeio.storage.settings.file_settings_store.get_file_store'
    ) as mock_get_store:
