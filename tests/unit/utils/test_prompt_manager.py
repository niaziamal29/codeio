...
from codeio.controller.state.control_flags import IterationControlFlag
from codeio.controller.state.state import State
from codeio.core.message import Message, TextContent
from codeio.events.observation.agent import MicroagentKnowledge
from codeio.microagent import BaseMicroagent
from codeio.utils.prompt import (
...
def prompt_dir(tmp_path):
    # Copy contents from "codeio/agenthub/codeact_agent" to the temp directory
    shutil.copytree(
        'codeio/agenthub/codeact_agent/prompts', tmp_path, dirs_exist_ok=True
    )
...
def test_jinja2_template_inheritance(prompt_dir):
    ...
    with open(os.path.join(prompt_dir, 'system_prompt.j2'), 'w') as f:
        f.write("""You are Codeio agent, a helpful AI assistant that can interact with a computer to solve tasks.
...
""")
    ...
    assert 'You are Codeio agent' in base_msg
    ...
    assert 'You are Codeio agent' in interactive_msg
    ...
    assert 'You are Codeio agent' in long_horizon_msg
...
Always be helpful and follow user instructions.
    ...
    assert 'You are Codeio agent' in cli_message
    ...
    assert 'You are Codeio agent' in sandbox_message
    ...
    assert 'You are Codeio agent' in default_message
