...
from codeio.events.action import FileReadAction, FileWriteAction
from codeio.events.observation import FileReadObservation, FileWriteObservation
...
def test_initialize_repository_for_runtime(temp_dir, runtime_cls, run_as_openhands):
    ...
    mock_repo = Repository(
        id='1232',
        full_name='Codeio/Codeio',
        git_provider=ProviderType.GITHUB,
        is_public=True,
    )

    with patch(
        'codeio.runtime.base.ProviderHandler.verify_repo_provider',
        return_value=mock_repo,
    ):
        repository_dir = initialize_repository_for_runtime(
            runtime, selected_repository='Codeio/Codeio'
        )

    assert repository_dir is not None
    assert repository_dir == 'Codeio'
