...
from codeio.events.action import Action, IPythonRunCellAction
from codeio.events.observation import IPythonRunCellObservation
from codeio.runtime.plugins.jupyter.execute_server import JupyterKernel
...
class JupyterPlugin(Plugin):
    ...
    async def initialize(
        ...
        if not is_local_runtime:
            ...
            poetry_prefix = (
                'cd /codeio/code\n'
                'export POETRY_VIRTUALENVS_PATH=/codeio/poetry;\n'
                'export PYTHONPATH=/codeio/code:$PYTHONPATH;\n'
                'export MAMBA_ROOT_PREFIX=/codeio/micromamba;\n'
                '/codeio/micromamba/bin/micromamba run -n openhands '
            )
        else:
            # LocalRuntime
            prefix = ''
