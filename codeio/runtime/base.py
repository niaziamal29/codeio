...
from codeio.events.serialization.action import ACTION_TYPE_TO_CLASS
...
from codeio.runtime.utils.edit import FileEditRuntimeMixin
from codeio.runtime.utils.git_handler import CommandResult, GitHandler
from codeio.security import SecurityAnalyzer, options
from codeio.storage.locations import get_conversation_dir
from codeio.utils.async_utils import (
    GENERAL_TIMEOUT,
...
class Runtime(FileEditRuntimeMixin):
    ...
    This is an extension point in Codeio that allows applications to customize how
    ...
    def maybe_run_setup_script(self):
        """Run .codeio/setup.sh if it exists in the workspace or repository."""
        setup_script = '.codeio/setup.sh'
    ...
    def maybe_setup_git_hooks(self):
        """Set up git hooks if .codeio/pre-commit.sh exists in the workspace or repository."""
        pre_commit_script = '.codeio/pre-commit.sh'
        ...
        if not isinstance(read_obs, ErrorObservation):
            # If the existing hook wasn't created by Codeio, preserve it
            if 'This hook was installed by Codeio' not in read_obs.content:
...
# This hook was installed by Codeio
...
"""
    ...
    @classmethod
    def teardown(cls, config: CodeioConfig):
