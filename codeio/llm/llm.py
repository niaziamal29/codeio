...
from codeio.llm.model_features import get_features
...
class LLM(RetryMixin, DebugMixin):
    ...
    def __init__(
        ...
        # Handle Codeio provider - rewrite to litellm_proxy
        if self.config.model.startswith('codeio/'):
            model_name = self.config.model.removeprefix('codeio/')
            self.config.model = f'litellm_proxy/{model_name}'
            self.config.base_url = _get_openhands_llm_base_url()
            logger.debug(
                f'Rewrote codeio/{model_name} to {self.config.model} with base URL {self.config.base_url}'
            )
        ...
        # Azure issue: https://github.com/Codeio/Codeio/issues/6777
