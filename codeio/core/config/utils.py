...
from codeio.core import logger
from codeio.core.config.agent_config import AgentConfig
from codeio.core.config.arg_utils import get_headless_parser
from codeio.core.config.condenser_config import (
    CondenserConfig,
    condenser_config_from_toml_section,
    create_condenser_config,
)
from codeio.core.config.extended_config import ExtendedConfig
from codeio.core.config.kubernetes_config import KubernetesConfig
from codeio.core.config.llm_config import LLMConfig
from codeio.core.config.mcp_config import MCPConfig
from codeio.core.config.model_routing_config import ModelRoutingConfig
...
def load_from_env(
    cfg: CodeioConfig, env_or_toml_dict: dict | MutableMapping[str, str]
) -> None:
    ...
    Args:
        cfg: The CodeioConfig object to set attributes on.
...
def load_from_toml(cfg: CodeioConfig, toml_file: str = 'config.toml') -> None:
    """Load the config from the toml file. Supports both styles of config vars.

    Args:
        cfg: The CodeioConfig object to update attributes of.
    ...
    # set LLMSummarizingCondenserConfig as default
        ...
        from codeio.core.config.condenser_config import LLMSummarizingCondenserConfig
...
def finalize_config(cfg: CodeioConfig) -> None:
...
def parse_arguments() -> argparse.Namespace:
    ...
    if args.version:
        print(f'Codeio version: {get_version()}')
        sys.exit(0)

    return args


def register_custom_agents(config: CodeioConfig) -> None:
...
def setup_config_from_args(args: argparse.Namespace) -> CodeioConfig:
    ...
    3. ~/.codeio/settings.json and ~/.codeio/config.toml
    """
