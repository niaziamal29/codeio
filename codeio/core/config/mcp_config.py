...
class CodeioMCPConfig:
    @staticmethod
    def add_search_engine(app_config: 'CodeioConfig') -> MCPStdioServerConfig | None:
        ...
        # Do not add search engine to MCP config in SaaS mode since it will be added by the Codeio server
        return None

    @staticmethod
    async def create_default_mcp_server_config(
        host: str, config: 'CodeioConfig', user_id: str | None = None
        ...
        """
        stdio_servers = []
        search_engine_stdio_server = CodeioMCPConfig.add_search_engine(config)
...
openhands_mcp_config_cls = os.environ.get(
    ...
    'codeio.core.config.mcp_config.CodeioMCPConfig',
)

CodeioMCPConfigImpl = get_impl(CodeioMCPConfig, openhands_mcp_config_cls)
