...
from codeio.agent_server.models import CodeioModel, SendMessageRequest
...
from codeio.integrations.service_types import ProviderType, SuggestedTask
from codeio.sdk.conversation.state import ConversationExecutionStatus
from codeio.sdk.llm import MetricsSnapshot
...
class AppConversationInfo(BaseModel):
    ...
    parent_conversation_id: CodeioUUID | None = None
    sub_conversation_ids: list[CodeioUUID] = Field(default_factory=list)
...
class AppConversationStartRequest(CodeioModel):
...
class AppConversationStartTask(CodeioModel):
    ...
    app_conversation_id: CodeioUUID | None = Field(
...
class AppConversationStartTaskPage(CodeioModel):
