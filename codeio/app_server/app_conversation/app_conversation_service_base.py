...
from codeio.app_server.sandbox.sandbox_models import SandboxInfo
from codeio.app_server.user.user_context import UserContext
from codeio.sdk import Agent
from codeio.sdk.context.agent_context import AgentContext
from codeio.sdk.context.condenser import LLMSummarizingCondenser
from codeio.sdk.context.skills import Skill
from codeio.sdk.llm import LLM
from codeio.sdk.security.analyzer import SecurityAnalyzerBase
from codeio.sdk.security.confirmation_policy import (
    AlwaysConfirm,
    ConfirmationPolicyBase,
    ConfirmRisky,
    NeverConfirm,
)
from codeio.sdk.security.llm_analyzer import LLMSecurityAnalyzer
...
def get_project_dir(
    ...
    `.codeio/` configuration (setup.sh, pre-commit.sh, skills/, etc.).
    ...
    - .codeio/setup.sh execution
    - .codeio/pre-commit.sh (git hooks setup)
    - .codeio/skills/ (project skills)
    - PLAN.md path

    Args:
        working_dir: Base working directory path in the sandbox
            (e.g., '/workspace/project' from sandbox_spec)
        selected_repository: Repository name (e.g., 'Codeio/software-agent-sdk')
...
class AppConversationServiceBase(AppConversationService, ABC):
    ...
    async def load_and_merge_all_skills(
        ...
        - Public skills (from Codeio/skills GitHub repo)
        - User skills (from ~/.codeio/skills/)
        - Organization skills (from {org}/.openhands repo)
        - Project/repo skills (from workspace .codeio/skills/)
    ...
    async def run_setup_scripts(
        ...
        # for all .codeio/ features (setup.sh, pre-commit.sh, skills).
    ...
    async def maybe_setup_git_hooks(
        ...
        """Set up git hooks if .codeio/pre-commit.sh exists in the project root.

        Args:
            workspace: Remote workspace for command execution.
            project_dir: Project root directory (repo root when a repo is selected).
        """
        command = 'mkdir -p .git/hooks && chmod +x .codeio/pre-commit.sh'
        ...
        with tempfile.TemporaryFile(mode='w+t') as temp_file:
            ...
            if result.get('success'):
                ...
                if 'This hook was installed by Codeio' not in temp_file.read():
