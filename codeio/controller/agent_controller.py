...
if TYPE_CHECKING:
    from codeio.security.analyzer import SecurityAnalyzer
...
from codeio.controller.state.state_tracker import StateTracker
...
from codeio.events.serialization.event import truncate_content
...
class AgentController:
    ...
    async def start_delegate(self, action: AgentDelegateAction) -> None:
        ...
        Codeio is a multi-agentic system. A `task` is a conversation between
        Codeio (the whole system) and the user, which might involve one or more inputs
        ...
        if action.runnable:
            if self.state.confirmation_mode and (
                ...
                if self.agent.config.cli_mode:
                    ...
                    # See issue: https://github.com/Codeio/Codeio/issues/10464
