...
from codeio.events.nested_event_store import NestedEventStore
from codeio.events.stream import EventStream
from codeio.integrations.provider import PROVIDER_TOKEN_TYPE, ProviderHandler
...
class DockerNestedConversationManager(ConversationManager):
    ...
    async def _create_runtime(
        ...
        env_vars['CONVERSATION_MANAGER_CLASS'] = (
            'codeio.server.conversation_manager.standalone_conversation_manager.StandaloneConversationManager'
        )
        ...
        if config.file_store == 'local':
            ...
            volumes.append(
                f'{file_store_path}/{conversation_dir}:/root/.codeio/{conversation_dir}:rw'
            )
