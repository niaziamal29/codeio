...
from codeio.integrations.azure_devops.azure_devops_service import (
    AzureDevOpsServiceImpl,
)
from codeio.integrations.bitbucket.bitbucket_service import BitBucketServiceImpl
from codeio.integrations.bitbucket_data_center.bitbucket_dc_service import (
    BitbucketDCServiceImpl,
)
from codeio.integrations.github.github_service import GithubServiceImpl
from codeio.integrations.gitlab.gitlab_service import GitLabServiceImpl
from codeio.integrations.provider import ProviderToken
from codeio.integrations.service_types import GitService, ProviderType
from codeio.server.shared import ConversationStoreImpl, config, server_config
from codeio.server.types import AppMode
from codeio.server.user_auth import (
    get_access_token,
    get_provider_tokens,
    get_user_id,
)
...
async def get_conversation_link(
    ...
    """Appends a followup link, in the PR body, to the Codeio conversation that opened the PR"""
...
async def create_pr(
    ...
    """Open a PR in GitHub"""
    logger.info('Calling Codeio MCP create_pr')

    request = get_http_request()
    headers = request.headers
    conversation_id = headers.get('X-Codeio-ServerConversation-ID', None)
...
async def create_mr(
    ...
    """Open a MR in GitLab"""
    logger.info('Calling Codeio MCP create_mr')
...
async def create_bitbucket_pr(
    ...
    logger.info('Calling Codeio MCP create_bitbucket_pr')
...
async def create_bitbucket_data_center_pr(
    ...
    logger.info('Calling Codeio MCP create_bitbucket_data_center_pr')
...
async def create_azure_devops_pr(
    ...
    logger.info('Calling Codeio MCP create_azure_devops_pr')
